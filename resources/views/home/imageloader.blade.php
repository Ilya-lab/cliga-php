...
<head>
    ...
    <link href="{{ asset('css/fileloader.css').'?v='.config('app.version') }}" rel="stylesheet">
    <link href="{{ asset('css/jcrop.min.css').'?v='.config('app.version') }}" rel="stylesheet">
    <link href="{{ asset('css/bootstrap.min.css').'?v='.config('app.version') }}" rel="stylesheet">
    <link href="{{ asset('js/datatables/buttons.bootstrap.min.css').'?v='.config('app.version') }}" rel="stylesheet" type="text/css" />

    <!-- CSRF Token -->
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <script src="{{ asset('js/jquery.min.js') }}"></script>
    <script src="{{ asset('js/imageloader.js').'?v='.config('app.version') }}"></script>
    <script src="{{ asset('js/jcrop/jcrop.min.js').'?v='.config('app.version') }}"></script>
</head>


<body>
<h2>Загрузка изображений</h2>
<form action="{{ url()->current() }}" method="post" id="my_form" enctype="multipart/form-data">
