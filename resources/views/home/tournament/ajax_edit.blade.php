                ...
                <form class="form-horizontal form-label-left">
                    <div class="form-group row">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input id="numberMatch" class="position-sticky" value="{{ $match->number }}">
                        </div>
                    </div>
                    <div class="form-group row">
                            ...
                            <select id="arenaMatch" class="position-sticky">
                                ...
                                @foreach($arenas as $arena)
                                    @if($arena->field && $arena->field->arena)
                                        <option value="{{ $arena->id }}" @if($arena->id == $match->arena_id) selected @endif>{{ $match->arena_id }} - {{ $arena->field->arena->name }} ({{ $arena->field->name }})</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select id="homeMatch" class="position-sticky">
                                ...
                                <option value="{{ $team->id }}" @if($team->id == $match->home_id) selected @endif>{{ $team->name }} ({{ $team->city }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select id="awayMatch" class="position-sticky">
                                <option value="0">Не выбрано</option>
                                @foreach($teams as $team)
                                    <option value="{{ $team->id }}" @if($team->id == $match->away_id) selected @endif>{{ $team->name }} ({{ $team->city }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
<script>
    ...
    var dt = new Date({{ Carbon\Carbon::parse($match->datetime)->format('U')*1000 }})
    @if($match->datetime == null) dt = null; @endif
    ...
    $('#editButton').on('click', function () {
        ...
        $.ajax({
            ...
            data: {id: {{ $match->id }}, 'datetime': $('#datetimeMatch').val(), 'arena_id': $('#arenaMatch').val(), 'number': $('#numberMatch').val(), 'home_id': $('#homeMatch').val(), 'away_id': $('#awayMatch').val() },
                ...
                if(gg) {
                    gg.find('td').each(function(cell){
                        if(cell == 0) $(this).html(response['match']['number'])
                            ...
                            if(response['match']['home']) $(this).html(response['match']['home']['name'])
