...
@section('title')Перечень соревнований @endsection
@section('js')
...
@component('components.main_block')
    @slot('title')Перечень соревнований @endslot
    @slot('description')список всех соревнований @endslot
    ...
    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap responsive-utilities jambo_table bulk_action" cellspacing="0" width="100%">
        ...
        <tbody>
        @foreach($tournlist as $tourn)
            <tr class="even pointer">
                <td class="align-content-center">{{ $loop->index + 1 }}</td>
                <td>{{ $tourn->name }}</td>
                <td>{{ $tourn->url }}</td>
                <td>{{ $tourn->league->name }}</td>
                <td><a onclick="if(confirm('Удалить соревнование?')) window.location.replace('{{ route('tournlist_delete', $tourn->id) }}');" style="cursor: pointer;">удалить</a></td>
            </tr>
        @endforeach
        </tbody>
    </table>
