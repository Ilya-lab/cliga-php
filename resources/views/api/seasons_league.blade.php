...
@section('title')Сезоны. Метод league. @endsection
@section('header')Метод league @endsection
@section('menu')
...
@section('content')
    <p>Метод <code translate="no" dir="ltr">league</code> ресурса <code translate="no" dir="ltr">seasons</code> возвращает все сезоны в лиге, с идентификатором <code translate="no" dir="ltr">id</code>. См.описание <a href="{{ route('api_doc_leagues') }}">лиги</a>.
        <a href="{{ route('api_seasons_leagues', 1) }}">Выполнить</a>.</p>

    <devsite-heading text="Запрос" for="properties_1" level="h4" link="" toc=""><h4 is-upgraded="" id="properties_1">Запрос</h4></devsite-heading>
    <pre id="alt-json" class="notranslate" style="max-height: 500px; overflow: auto" dir="ltr" is-upgraded="">{{ route('api_seasons_leagues', 1) }}</pre>
