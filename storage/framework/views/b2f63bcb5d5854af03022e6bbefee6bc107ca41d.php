...
<?php $__env->startSection('css'); ?>
    ...
    <link href="<?php echo e(asset('js/toggle/css/bootstrap4-toggle.min.css')); ?>" rel="stylesheet">
    <!-- datetimepicker -->
    <link href="<?php echo e(asset('css/datetimepicker/bootstrap-datetimepicker.min.css')); ?>" rel="stylesheet">
    <!-- dropzone -->
    <!-- <link href="<?php echo e(asset('css/dropzone/dropzone.min.css')); ?>" rel="stylesheet"> -->
<?php $__env->stopSection(); ?>
...
<?php $__env->startSection('js_footer'); ?>
    ...
    <script src="<?php echo e(asset('js/toggle/js/bootstrap4-toggle.min.js')); ?>"></script>

    <!-- datetimepicker -->
    <script src="<?php echo e(asset('js/datetimepicker/moment-with-locales.min.js')); ?>"></script>
    <script src="<?php echo e(asset('js/datetimepicker/bootstrap-datetimepicker.min.js')); ?>"></script>
    <!-- dropzone -->
    <script src="<?php echo e(asset('js/dropzone/dropzone.js?v14')); ?>"></script>
    ...
    <script>
        ...
        (function() {
            ...
            $(".dropzone").dropzone({
                url: '<?php echo e(route('loadNewsPhoto')); ?>',
                ...
                success: function(res, index){
                    ...
                    if(rs['status'] == 'OK') {
                        ...
                        $('#image_preview img').attr('src', '<?php echo e(env("APP_URL")); ?>' + '/' + rs['files'][0]);
        ...
        function arjNews(id) {
            if(confirm('Поместить новость в архив?')) {
                $.ajax({
                    ...
                    url: '<?php echo e(url()->current()); ?>/archive',
                    data: {
                        "id": id
                    }
                }).done(function(response) {
            ...
            $.ajax({
                ...
                if(response['status'] == 'OK') {
                    ...
                    if(response['news']['image2'].trim().length > 0) {
                        ...
                        $('#image_preview').show();
                        $('#image_preview').attr('class','visible');
                        $('#image_preview img').attr('src', '<?php echo e(env("APP_URL")); ?>' + '/' + response['news']['image2']);
...
<?php $__env->startComponent('components.main_block'); ?>
    ...
    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap responsive-utilities jambo_table bulk_action" cellspacing="0" width="100%">
        ...
        <?php $__currentLoopData = $news; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nw): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr class="even pointer" id="news<?php echo e($nw->id); ?>" image="<?php echo e($nw->image2); ?>">
                <td class="align-content-center"><?php echo e($loop->index + 1); ?></td>
                <td>
                    <button class="btn-info news-edit" attr-id="<?php echo e($nw->id); ?>">изменить</button>
                    <button class="btn-danger news-del" attr-id="<?php echo e($nw->id); ?>">удалить</button>
                    <button class="btn-warning news-arj" attr-id="<?php echo e($nw->id); ?>">архив</button>
                </td>
                <td><?php echo e($nw->title); ?></td>
                <td><?php echo e($nw->desc); ?></td>
                <td><?php echo e($nw->name); ?></td>
                <td><?php echo e($nw->date); ?></td>
            </tr>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </tbody>
    </table>
